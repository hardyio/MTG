...
public class PoemHeader extends LinearLayout implements RefreshHeader {
    ...
    public int onFinish(@NonNull RefreshLayout layout, boolean success) {
        ...
        if (success) {
            mHeaderText.setText(StringUtils.isEmpty(poem) ? "Refresh complete" : poem);
        }
        else {
            mHeaderText.setText(StringUtils.isEmpty(poem) ? "Refresh fail" : poem);
        }
    ...
    public void onStateChanged(@NonNull RefreshLayout refreshLayout, @NonNull RefreshState oldState,
        ...
        switch (newState) {
            ...
            case PullDownToRefresh:
                mHeaderText.setText(StringUtils.isEmpty(poem) ? "Pull down to refresh" : poem);
                mArrowView.setVisibility(VISIBLE);//显示下拉箭头
                mProgressView.setVisibility(GONE);//隐藏动画
                mArrowView.animate().rotation(0);//还原箭头方向
                break;
            case Refreshing:
                mHeaderText.setText(StringUtils.isEmpty(poem) ? "Loading..." : poem);
                mProgressView.setVisibility(VISIBLE);//显示加载动画
                mArrowView.setVisibility(GONE);//隐藏箭头
                break;
            case ReleaseToRefresh:
                mHeaderText.setText(StringUtils.isEmpty(poem) ? "Release to refresh" : poem);
